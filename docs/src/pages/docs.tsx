    ...
    <Layout title="Sesh Documentation">
      <div css={baseStyles}>
        ...
        <p>
          ...
          <code>Authorization: Bearer YOUR_API_KEY</code>. Request data can be
          submitted in query parameters or in the request body (for non-GET
          requests), and all request bodies should be in JSON format. All
          responses will return valid JSON unless they return <code>null</code>,
          in which case the response body will be empty.
        </p>
        <p>
        ...
        <EndpointDocumentation
          ...
          headingId="creating-sessions"
          description={
            <>
              Creates a session with a securely-generated 16-byte alphanumeric
              token.
            </>
          }
        ...
        />
        <EndpointDocumentation
          heading="Reading/Validating a Session"
          headingId="reading-sessions"
          ...
          parameters={{
            ...
            fields: [
              { key: "token", type: "string" },
              {
                ...
                comment: (
                  <>
                    The IP address of the user using this session to perform an
                    action. Required to match session&rsquo;s IP if the session
                    has an IP defined.
                  </>
                ),
                optional: true,
              },
              {
                ...
                comment: (
                  <>
                    The client ID (any value, such as a browser fingerprint) of
                    the user using this session to perform an action. Required
                    to match session&rsquo;s client ID if the session it
                    defined.
                  </>
                ),
                optional: true,
          ...
          returnDescription={
            <>
              A session if one is found, or a 404 error if one is not or the IP
              or client ID do not match.
            </>
          }
        />
        <EndpointDocumentation
          heading={<>Refreshing a Session&rsquo;s Expiration</>}
          headingId="refreshing-sessions"
          description={
            <>
              Sets the session&rsquo;s expiration to be in a date as far in the
              future from now as its original expiration date was from the time
              it was created. Essentially, this resets the session&rsquo;s
              time-to-live.
            </>
          }
        ...
        />
        <EndpointDocumentation
          heading={<>Updating a Session&rsquo;s Metadata</>}
          headingId="updating-metadata"
          description={
            <>
              Updates a session&rsquo;s metadata to any new JSON data desired.
            </>
          }
